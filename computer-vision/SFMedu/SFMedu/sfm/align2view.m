function pair = align2view(frames, frameID_i, frameID_j)

% SFMedu: Structrue From Motion for Education Purpose
% Written by Jianxiong Xiao (MIT License)

%% load two images and depths
image_i=imread(frames.images{frameID_i});
image_j=imread(frames.images{frameID_j});
...
[F, inliers] = ransacfitfundmatrix(SIFTloc_i, SIFTloc_j, t, 1);
        
%t = .02;  % Distance threshold for deciding outliers
%[F, inliers] = ransacfitfundmatrix7(SIFTloc_i, SIFTloc_j, t, 1);

fprintf('%d inliers / %d SIFT matches = %.2f%%\n', length(inliers), size(SIFTloc_i,2), 100*length(inliers)/size(SIFTloc_i,2));

SIFTloc_i = SIFTloc_i(:,inliers);
SIFTloc_j = SIFTloc_j(:,inliers);
